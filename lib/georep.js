 ...
 * @param {User} user istanza di un oggetto User 
 * @param {function} callback funzione che viene eseguita al termine dell'interazione con il server.
 */
Georep.prototype.signupRemoteUser = function(callback, user){
	if (arguments.length == 0){
		throw {
			error: 'Serve almeno un parametro',
			args: arguments
		};
	} else if( arguments.length == 1 && typeof callback != 'function' ) {
		throw {
			error: 'Il parametro deve essere una funzione',
			args: callback
		};
	} else if ( arguments.length > 1 && !(user instanceof User)){
		throw {
			error: 'Parametro user non valido',
			args: user
		};
	} else if (arguments.length > 1 && typeof callback != 'function'){
		throw {
			error: 'Parametro callback non valido',
			args: callback
		};
	} else {
		...
		if( arguments.length == 1){
			...
			var usersignup = {
					_id: "org.couchdb.user:" + user.getName(),
					 name: user.getName(),
 ...
 * @return {String} URL al quale inviare la richiesta; viene inviata al server couchdb se <b>non</b> siamo
 * in appMode, altrimenti viene inviata al server nodeJS che controlla la registrazione.
 */
var makeSignupUrlRequest = function (georepObj) {
	if (constants.appMode) {
		...
		return georepObj.getDb().getURLServer() + '/_users';
	}
};
...
var constants = {
	...
	// specifica se il modulo è utilizzato nella app o nel server node.js
	appMode: false,
	// se appMode == true allora queste info sono utilizzate per comporre l'URL
	// utilizzato nell'invio delle richieste di signup 
	nodeJsServer: {
		proto: 'http',
